classdef sleepAnalysis < recAnalysis
    ...
    methods
        ...
        function data=getLizardMovements(obj,varargin)
            ...
            startTimes=tStart:(movLongWin-movLongOL):endTime;
            ...
            staticSamples=obj.filt.FFs*staticWin/1000;
            ...
            for i=1:nChunks
                ...
                bufferedHighpass=buffer(allAxesHighpass,1500,0,'nodelay');
                bufferedEnvelope=buffer(allAxes,1500,0,'nodelay');
                ...
                %head pitch and roll angle evaluation for static periods
                ThStatic=noiseMean+staticDetectionThresholdStd*noiseStd;
                staticBinary=abs(allAxes)<ThStatic;
                staticBinary=convn(staticBinary,ones(staticSamples,1),'same')==staticSamples;

                %figure;plot(t_ms,squeeze(allAxes));hold on;plot(t_ms(staticBinary),squeeze(allAxes(staticBinary)),'.r');
            ...
            for j = p
                xfm = makehgtform('xrotate',angles(1,j),'yrotate',angles(2,j),'zrotate',angles(3,j));
                newdir = xfm * dir';
                h.UData = newdir(1);
                h.VData = newdir(2);
                h.WData = newdir(3);
        ...
        function data=getSyncedDBEyeMovements(obj,varargin)
            ...
            if ~isPreyTouch %previous version without trigger blinks for the videos start/end
                ...
                camTrigFile=[obj.currentAnalysisFolder filesep 'camTrigs.mat'];
                obj.checkFileRecording(camTrigFile,'camera trigger file missing, please first run getCameraTriggers');
                load(camTrigFile)
                tFrames = camTrigs;
        ...
        function [data]=getEyeMovements(obj,varargin)
            ...
            addParameter(parseObj,'opticFlowNoiseThreshold',0.0039,@isnumeric);%0.035
            ...
            if removeBorderOF
                ...
                fprintf('Removing boarder of %d pixels for optic flow analysis, removing a fraction of %f samples.\n',borderPix,numel(pBorder)/initialFrameSubregion(3)/initialFrameSubregion(4));
            end
            ...
            for i=1:nFrames
                ...
                % this is faster but cant start from an arbitrary frame or jump frames
        ...
        function arenaCSVs = getArenaCSVs(obj,overwrite)
            ...
            if length(oeCamTrig) == length(arenaCSVs.videoFrames)
                disp('Triggers num match video frames.')
            else
                framediff = length(arenaCSVs.videoFrames)-length(oeCamTrig);
            ...
            arenaCSVs.IRFrames = [IRframeShift, oeIRFrame, arenaIRFrame];
            ...
            if exist(screenTouchFile,'file') ==2 %make sure there were screen touches:
                ...
                % arenaCSVs.screenTouch = [];
                % arenaCSVs.strikeFrameSh = [];
                % arenaCSVs.strikeTrialNum = [];
                % arenaCSVs.strikeTrigSh =[];
            end
        ...
        function stimTriggers = getStimTriggers(obj,overwrite)
            ...
            if exist(stimTriggerPath, 'file') && ~overwrite
                if nargout > 0
                    ...
                    if isfield(data, 'stimTriggers')
                        stimTriggers = data.stimTriggers;
                    elseif isfield(data, 'diodeTriggers')
                        stimTriggers = data.diodeTriggers;
                    else
                        % Return the first field if naming is different
                        fields = fieldnames(data);
                        stimTriggers = data.(fields{1});
                    end
                end
                return;
            end
            ...
            elseif ~isnan(obj.recTable.StimTrighCh(obj.currentPRec))
                ...
                stimCh =  obj.recTable.StimTrighCh(obj.currentPRec);
                obj.getDigitalTriggers;
                T = obj.getDigitalTriggers;
                stimTriggers = T.tTrig{stimCh};
                % Save as stimTrigger.mat
                save(stimTriggerPath, 'stimTriggers');
            ...
            % If we reach here and output is requested, throw an error
            if nargout > 0
                error('Unable to find or generate stimulus triggers for current recording.');
            end
        end
        ...
        function [diodeTriggers]=getStimDiodeTrig(obj,varargin)
            ...
            % --- Find large gaps ---
            bigGaps = find(t_diff > maxDiff);

            if isempty(bigGaps)
                % no large gaps > keep all
                diodeTriggers = trigTimes;
            else
                % --- Handle start ---
                % last big gap before the 8 small-diffs pattern begins
                firstKeepIdx = bigGaps(find(bigGaps < numel(trigTimes)/2, 1, 'last')) + 1;
                if isempty(firstKeepIdx)
                    firstKeepIdx = 1; % no junk at start
                end

                % --- Handle end ---
                % first big gap after the last 8 small-diffs pattern
                lastKeepIdx = bigGaps(find(bigGaps > numel(trigTimes)/2, 1, 'first'));
                if isempty(lastKeepIdx)
                    lastKeepIdx = numel(trigTimes); % no junk at end
                end

                % --- Extract only the valid block ---
                diodeTriggers = trigTimes(firstKeepIdx:lastKeepIdx);
            end
        ...
        %% getCameraTriggers
        function camTrigs = getCameraTriggers(obj,overwrite,stop_len)
...
%% plotDBtwitchesRaster
        function hOut= plotLizardMovementRaster(obj,varargin)
            %% parameter and settings
            hOut=[];
            obj.checkFileRecording;

            parseObj = inputParser;
            parseObj.FunctionName='sleepAnalysis\plotLizardMovementRaster';
            addParameter(parseObj,'ch',obj.recTable.defaulLFPCh(obj.currentPRec),@isnumeric);
            addParameter(parseObj,'nBins',18,@isnumeric);
            addParameter(parseObj,'videoFile',[obj.recTable.VideoFiles{obj.currentPRec}],@(x) exist(x,'file'));
            addParameter(parseObj,'saveFigures',1,@isnumeric);
            ...
            addParameter(parseObj,'inputParams',false,@isnumeric);
            addParameter(parseObj,'stim',0,@isnumeric);
            addParameter(parseObj,'part',1,@isnumeric);
            ...
            %make parameter structure
            parPlotDBTwitch=parseObj.Results;
            ...
            if stim ==1
                stimFile=[obj.currentAnalysisFolder filesep 'stimTrigger.mat'];
                obj.checkFileRecording(stimFile,'stim file missing, please first run getStimTriggers');
                load(stimFile); %load data
                
                tStartStim = stimTriggers(1);
                tEndStim = stimTriggers(end);
            ...
            if numel(h)>=1
                edges=(0:nBins)/(nBins-0.0000001);
                middles=(edges(1:end-1)+edges(2:end))/2;

                axes(h(1));
                hOut.imagesc=imagesc(0:nBins,1:size(resampledTemplate,1),resampledTemplate,cLim);hold on;
                set(h(1),'XTickLabel',[]);
                ylabel('# cycle');
                p=cell2mat(cellfun(@(x) ~isempty(x),phaseAll,'UniformOutput',0));
                for i=find(p)
                    hOut.hP(i)=plot(phaseAll{i}*nBins,i*ones(size(phaseAll{i})),'.r');
                end
                phaseMov=cell2mat(phaseAll);
                I=histc(phaseMov,edges);
                ...
                if numel(h)==1
                    xlabel('Phase');
                    set(h,'XTick',[-0.5 nBins+0.5],'XTickLabel',{'0','2\pi'});
                    xlim([-0.5 nBins+0.5])
                end
            end
            ...
            if numel(h)>=2
                ...
                xlabel('Phase');
                set(h(2),'XTick',[0 1],'XTickLabel',{'0','2\pi'});
                hOut.l=legend('norm. \delta/\beta','norm. mov counts');
                hOut.l.Box='off';
                hOut.l.Position=[0.1    0.9061    0.2596    0.0812];
            end
            ...
            if numel(h)>=3
                ...
                p=cell2mat(cellfun(@(x) ~isempty(x),phaseAllRand,'UniformOutput',0));
                for i=find(p)
                    plot(phaseAllRand{i}*nBins,i*ones(size(phaseAllRand{i})),'*r','MarkerSize',2);
                end
                phaseRand=cell2mat(phaseAllRand);
                I=histc(phaseRand,edges);
                title('Randomized');
            ...
            if saveFigures
                set(fH,'PaperPositionMode','auto');
                fileName=[obj.currentPlotFolder filesep 'LizardMovRaster'];
                print(fileName,'-dpdf',['-r' num2str(obj.figResJPG)]);
                if printLocalCopy
                    fileName=[cd filesep obj.recTable.Animal{obj.currentPRec} '_Rec' num2str(obj.currentPRec) '_LizardMovRaster'];
                    print(fileName,'-dpdf',['-r' num2str(obj.figResJPG)]);
                end
            end
